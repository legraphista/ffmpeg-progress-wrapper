...
Ffmpeg.prototype.kill = function(signal = 'SIGKILL') {
...
Ffmpeg.prototype.done = function(callback) {
    var self = this;

    if(typeof callback !== 'function') {
        return new Promise(function (res, rej) {
            return self.done(function(e, d) {
                if(e) {
                    return rej(e);
                }
                return res(d);
    ...
    this._process.once('close', function(code, signal) {
        // SIGINT allow ffmpeg use graceful shutdown
        if(signal === self._isKilledByUser && signal === 'SIGINT') {
            return callback(null, self._output);
        }

        if(code || signal){
            var e = new Error('FFmpeg closed unexpectedly');
            e.code = code;
            e.signal = signal;
            e.args = self._args;
            e.killedByUser = signal === self._isKilledByUser;
            return callback(e);
        }
...
Ffmpeg.prototype.processOutput = function(buffer) {
    ...
    var isMetadataDuration;
    if(isMetadataDuration = text.toLowerCase().match(/duration\s+: ((\d+:?){1,3}.\d+)/)) {
    ...
    var isFrame;
    if (isFrame = text.match(/(frame|time)=.*/)) {
...
Ffmpeg.prototype.processProgress = function(text) {
    var duration = this.options.duration || (this._details.file && this._details.file.duration) || this._metadataDuration || null;
    var data = text
        .trim()
        .replace(/=\ */g, '=')
        .split(' ')
...
Ffmpeg.prototype.processMetadataDuration = function(humanDuration) {
    this._metadataDuration = Math.max(this._metadataDuration, helper.humanTimeToMS(humanDuration));
};
