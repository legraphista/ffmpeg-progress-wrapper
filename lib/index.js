...
Ffmpeg.prototype.processOutput = function(buffer) {
    ...
    if (!this._details.file && ~this._output.toLowerCase().search(/duration.*\n/i))
...
Ffmpeg.prototype.processProgress = function(text) {
    ...
    // compute progress
    data.progress = data.time / this._details.file.duration;

    // compute ETA
    data.eta = ((this._details.file.duration - data.time) / data.speed) | 0;
